...
function config_url {
    echo "https://dl.cloudsmith.io/public/isc/kea-2-4/config.deb.txt?distro=${distro}&codename=${codename}&version=${version}&arch=${arch}" | sed 's/ /%20/g'
}
...
function import_gpg_key {
    local text="Importing 'isc/kea-2-4' repository GPG keys ..."
    echo_running "$text"

    local gpg_keyring_path="/usr/share/keyrings/isc-kea-2-4-archive-keyring.gpg"

    curl -1sLf "https://dl.cloudsmith.io/public/isc/kea-2-4/gpg.0D9D9A1439E23DB9.key" | gpg --dearmor >> $gpg_keyring_path
    ...
    echo_okfail "Checking for apt signed-by key support ..." || {
        mv ${gpg_keyring_path} /etc/apt/trusted.gpg.d/isc-kea-2-4.gpg
    }
...
function setup_repository {
    local repo_path="/etc/apt/sources.list.d/isc-kea-2-4.list"
    check_fetch_config

    local text="Installing 'isc/kea-2-4' repository via apt ..."
    echo_running "$text"
...
echo "Executing the  setup script for the 'isc/kea-2-4' repository ..."
echo
